        ...
        <div className="pg-page-body">
          ...
          <div className="pg-stack pg-stack--sm">
            {opts.map((o) => (
              <button
                key={o.id}
                className={`pg-choice${state.cargo === o.id ? ' is-active' : ''}`}
...
// =====================================================================
type AccessBlockValue = NonNullable<PagoraState['originAccess']>;
const AccessBlock = ({
  label,
  value,
  onChange,
  ...
  label: string;
  value: AccessBlockValue | undefined;
  onChange: (next: AccessBlockValue) => void;
}) => {
  const v: AccessBlockValue = value ?? {};
  const set = (patch: Partial<AccessBlockValue>) => onChange({ ...v, ...patch });
  return (
    <div className="pg-stack pg-stack--sm">
      <div className="pg-segmented">
        {[
          ...
          <button
            key={o.id}
            className={`pg-segmented-item${v.type === o.id ? ' is-active' : ''}`}
            onClick={() => set({ type: o.id as 'house' | 'apt' })}
          >
            {o.t}
          </button>
        ))}
      ...
      {v.type === 'apt' && (
        <div className="pg-row" style={{ gap: 10 }}>
          ...
          >
            ...
            <div className="pg-choice-body">
              <div className="pg-choice-title" style={{ fontSize: 14 }}>
                Tem elevador
              </div>
            </div>
          </button>
      ...
      <div className="pg-page">
        <div className="pg-page-body">
          ...
          >
            ...
            <AccessBlock
              label="carregar"
              value={state.originAccess}
              onChange={(v) => set({ originAccess: v })}
            />
          </div>
          ...
          >
            ...
            <AccessBlock
              label="descarregar"
              value={state.destAccess}
              onChange={(v) => set({ destAccess: v })}
            />
          </div>
        </div>
...
// =====================================================================
const Frete3 = ({ go, state, set }: FlowScreenProps) => {
  const veh = [
        ...
        <div className="pg-page-body">
          <div>
            ...
            <p className="pg-h-sub">
              Em dúvida? Escolha o tamanho médio — o prestador confirma na proposta.
            </p>
          </div>
          ...
          <div className="pg-stack pg-stack--sm">
            {veh.map((v) => (
              <button
                key={v.id}
                className={`pg-tile${state.vehicle === v.id ? ' is-active' : ''}`}
          ...
          <div className="pg-card pg-card--padded">
            <div className="pg-row pg-row--between">
              ...
              <div className="pg-row" style={{ gap: 6 }}>
                ...
                <span
                  className="pg-mono"
                  style={{ minWidth: 24, textAlign: 'center', fontWeight: 700, fontSize: 20 }}
                >
                  {helpers}
                </span>
                <button
        ...
        <div className="pg-page-body">
          ...
          <div className="pg-field">
            ...
            <textarea
              ...
              placeholder="Ex.: geladeira grande, sofá 3 lugares, peças desmontadas, piano…"
              value={state.notes || ''}
...
const FreteSummary = ({ go, state }: SummaryScreenProps) => {
  ...
  const accessLabel = (a: PagoraState['originAccess']): string => {
    ...
    const map: Record<string, string> = {
      house: 'Casa térrea',
      apt: `Apartamento, ${a.floor || '—'}º andar`,
      comm: 'Comércio',
    };
    return (
      (map[a.type] ?? '—') +
      (a.type === 'apt' ? (a.elevator ? ' · com elevador' : ' · sem elevador') : '')
    );
  };
  ...
  const cargo = cargoMap[state.cargo ?? ''] ?? '—';
  const veh = vehicleMap[state.vehicle ?? ''] ?? '—';
  const urg = urgencyMap[state.urgency ?? ''] ?? '—';
  return (
        ...
        <div className="pg-page-body">
          ...
          >
            <div
              style={{
                ...
                height: 140,
                background:
                  'radial-gradient(circle at top right, rgba(34,227,163,0.18), transparent 60%)',
              }}
            />
            ...
            <div className="pg-stack pg-stack--sm" style={{ fontSize: 14 }}>
              {((): ReadonlyArray<readonly [string, string, string]> => {
                const rows: Array<readonly [string, string, string]> = [
                  ...
                  [
                    'Ajudantes',
                    `${state.helpers || 0} pessoa(s)`,
                    `R$ ${price.breakdown.helpers ?? 0}`,
                  ],
                  [
                    'Acesso difícil',
                    state.originAccess?.needHelp || state.destAccess?.needHelp ? 'Sim' : 'Não',
                    `R$ ${price.breakdown.access ?? 0}`,
                  ],
                  [
                    'Sem elevador',
                    price.breakdown.noElev ? 'Sim' : 'Não',
                    `R$ ${price.breakdown.noElev ?? 0}`,
                  ],
                  ['Taxa base', 'Plataforma', `R$ ${price.breakdown.baseFee}`],
                ];
                if (state.urgency === 'today')
                  rows.push(['Urgência', 'Hoje', price.breakdown.urgency]);
                return rows;
              })().map((row, i) => (
                <div
                  key={i}
          ...
          >
            ...
            <div style={{ fontSize: 13, color: 'var(--ink-800)', lineHeight: 1.5 }}>
              <strong>Valor pode variar conforme:</strong> volume real da carga, dificuldade no
              local, necessidade de embalagem extra, e disponibilidade no horário escolhido.
            </div>
          </div>
          ...
          <div className="pg-card pg-card--padded">
            ...
            <div className="pg-stack pg-stack--sm" style={{ marginTop: 14, fontSize: 14 }}>
              {((): ReadonlyArray<readonly [string, string, string | null]> => {
                ...
                if (state.notes) rows.push(['Observações', state.notes, 'edit']);
                return rows;
              })().map((row, i) => (
...
// CONFIRMATION
// =====================================================================
const FreteConfirm = ({ go, state, reset }: ConfirmScreenProps) => {
    ...
    <div className="pg-screen" data-screen-label="08 Frete · Enviado">
      ...
      <div className="pg-viewport">
        <div style={{ padding: '8px 24px 24px', textAlign: 'center' }}>
          ...
          >
            Seu pedido <span className="pg-mono pg-bold">#PG-1247</span> foi enviado para
            prestadores especializados na sua região.
          </p>
        </div>
          ...
          <div className="pg-card pg-card--padded">
            ...
            <div style={{ marginTop: 16, position: 'relative' }}>
              {[
                ...
                {
                  t: 'Prestadores avaliando',
                  s: 'Equipamento, rota, disponibilidade',
                  done: false,
                  current: true,
                },
                    ...
                    style={{
                      ...
                      background: step.done
                        ...
                        : step.current
                          ...
                          : 'var(--paper)',
                      border:
                        step.done || step.current ? 'none' : '1.5px solid var(--border-strong)',
                      color: step.done
                        ? 'var(--night-900)'
                        : step.current
                          ? 'var(--green-500)'
                  ...
                  <div>
                    <div
                      style={{
                        fontSize: 14,
                        fontWeight: 600,
                        color: step.done || step.current ? 'var(--text)' : 'var(--text-mute)',
                      }}
                    >
                      {step.t}
                    </div>
...
// =====================================================================
const Stub = ({ go, title, sub }: { go: (route: string) => void; title: string; sub?: string }) => (
  <div className="pg-screen" data-screen-label={`Stub · ${title}`}>
    ...
    <div className="pg-page">
      ...
      >
        ...
        <button className="pg-btn pg-btn--ghost" onClick={() => go('services')}>
...
export { Frete1, Frete2, Frete3, Frete4, FreteSummary, FreteConfirm };
