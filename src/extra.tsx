...
// Pricing
const calcGuincho = (s: PagoraState): PricingResult => {
  ...
  const ACCESS: Record<string, number> = { rua: 0, garagem: 40, dificil: 80, expressa: 120 };
  const veh = VEHICLE[s.vehicleType ?? ''] ?? 30;
  const access = ACCESS[s.location ?? ''] ?? 0;
  const baseFee = 60;
  ...
  return {
    ...
    breakdown: { baseKm, veh, access, baseFee, urgency: s.urgency === 'now' ? '+50%' : '—' },
  };
};

const calcCacamba = (s: PagoraState): PricingResult => {
  const SIZE: Record<string, number> = { p3: 180, p5: 280, p8: 380 };
  const TIME: Record<string, number> = { d1: 0, d3: 60, d7: 120 };
  const sizePrice = SIZE[s.size ?? ''] ?? 280;
  const time = TIME[s.duration ?? ''] ?? 0;
  const total = sizePrice + time;
  return {
    low: total,
    high: Math.round(total * 1.15),
    breakdown: { baseKm: 0, sizePrice, time, baseFee: 0, urgency: '—' },
  };
};
...
const Guincho1 = ({ go, state, set }: FlowScreenProps) => {
  const opts = [
    {
      id: 'pane',
      t: 'Pane mecânica / Não liga',
      s: 'Carro travou, motor não dá partida',
      icon: 'wrench',
    },
    {
      id: 'pneu',
      t: 'Pneu furado',
      s: 'Pode não precisar guincho — confirme com prestador',
      icon: 'alert',
    },
    {
      id: 'acidente',
      t: 'Acidente / Batida',
      s: 'Guincho especial pode ser necessário',
      icon: 'alert',
    },
    {
      id: 'combustivel',
      t: 'Falta de combustível',
      s: 'Reabastecimento ou guincho até posto',
      icon: 'fuel',
    },
    {
      id: 'chave',
      t: 'Chave quebrada / Trancado',
      s: 'Chaveiro ou remoção do veículo',
      icon: 'key',
    },
        ...
        <div className="pg-page-body">
          <div>
            ...
            <p className="pg-h-sub">
              Cada situação exige um equipamento diferente — isso ajuda o prestador a chegar
              preparado.
            </p>
          </div>
          <div className="pg-stack pg-stack--sm">
            {opts.map((o) => (
              <button
                key={o.id}
                className={`pg-choice${state.problem === o.id ? ' is-active' : ''}`}
...
// =====================================================================
const Guincho2 = ({ go, state, set }: FlowScreenProps) => {
  const opts = [
      ...
      <div className="pg-page">
        <div className="pg-page-body">
          <div>
            ...
            <p className="pg-h-sub">
              SUVs e pickups exigem plataforma reforçada. Motos vão em asa-delta.
            </p>
          </div>
          <div className="pg-stack pg-stack--sm">
            {opts.map((o) => (
              <button
                key={o.id}
                className={`pg-choice${state.vehicleType === o.id ? ' is-active' : ''}`}
          ...
          <div className="pg-field">
            ...
            <textarea
              ...
              placeholder="Ex.: Civic 2015 prata, placa ABC-1234, está travado em garagem subterrânea"
...
// =====================================================================
const Guincho3 = ({ go, state, set }: FlowScreenProps) => {
  const locOpts = [
    { id: 'rua', t: 'Na rua', s: 'Fácil acesso', icon: 'navigation' },
    {
      id: 'garagem',
      t: 'Garagem / estacionamento',
      s: 'Pode exigir manobra especial',
      icon: 'building',
    },
    {
      id: 'dificil',
      t: 'Local de difícil acesso',
      s: 'Subsolo, ladeira, espaço apertado',
      icon: 'alert',
    },
    {
      id: 'expressa',
      t: 'Via expressa / rodovia',
      s: 'Requer cuidado e sinalização',
      icon: 'bolt',
    },
  ];
  const destOpts = [
        ...
        <div className="pg-page-body">
          ...
          >
            <div className="pg-row" style={{ gap: 8 }}>
              ...
              <span className="pg-h-eyebrow" style={{ margin: 0 }}>
                LOCAL ATUAL DO VEÍCULO
              </span>
            </div>
            <div className="pg-input-wrap">
              ...
              <input
                ...
                placeholder="Endereço onde está o veículo"
                value={state.currentLoc || ''}
                onChange={(e) => set({ currentLoc: e.target.value })}
              />
            </div>
            <div className="pg-stack pg-stack--sm">
              {locOpts.map((o) => (
                <button
                  key={o.id}
                  className={`pg-choice${state.location === o.id ? ' is-active' : ''}`}
                ...
                >
                  <span
                    style={{
                      ...
                      flexShrink: 0,
                      background:
                        state.location === o.id ? 'rgba(255,255,255,0.1)' : 'var(--ink-100)',
          ...
          >
            ...
            <div className="pg-stack pg-stack--sm">
              {destOpts.map((o) => (
                <button
                  key={o.id}
                  className={`pg-choice${state.destType === o.id ? ' is-active' : ''}`}
            ...
            {state.destType && state.destType !== 'sugerir' && (
              <div className="pg-input-wrap">
                ...
                <input
                  className="pg-input pg-input--with-icon"
                  placeholder={
                    state.destType === 'oficina' ? 'Endereço da oficina' : 'Endereço da residência'
                  }
...
// =====================================================================
const Guincho4 = ({ go, state, set }: FlowScreenProps) => {
  ...
  const opts = [
    ...
    {
      id: 'schedule',
      t: 'Agendar para outra hora',
      s: 'Para casos não emergenciais',
      icon: 'calendar',
    },
  ];
  return (
        ...
        <div className="pg-page-body">
          ...
          <div className="pg-stack pg-stack--sm">
            {opts.map((o) => (
              <button
                key={o.id}
                className={`pg-choice${state.urgency === o.id ? ' is-active' : ''}`}
          ...
          >
            <div
              style={{
                ...
                height: 140,
                background:
                  'radial-gradient(circle at top right, rgba(255,122,26,0.18), transparent 60%)',
              }}
            />
            <div className="pg-h-eyebrow" style={{ color: 'var(--orange-500)', margin: 0 }}>
              VALOR ESTIMADO
            </div>
            <div
              style={{
            ...
            >
              R$ {price.low}
              <span style={{ color: 'rgba(255,255,255,0.4)' }}> – </span>R$ {price.high}
            </div>
            <hr
              className="pg-divider--dashed"
              style={{ margin: '16px 0', borderColor: 'rgba(255,255,255,0.16)' }}
            />
            <div className="pg-stack pg-stack--sm" style={{ fontSize: 14 }}>
              {((): ReadonlyArray<readonly [string, string, string]> => {
                const rows: Array<readonly [string, string, string]> = [
                  ...
                  ['Tipo de veículo', '—', `R$ ${price.breakdown.veh ?? 0}`],
                  ['Local', '—', `R$ ${price.breakdown.access ?? 0}`],
                  ['Taxa base', '—', `R$ ${price.breakdown.baseFee}`],
                ];
                if (state.urgency === 'now')
                  rows.push(['Urgência', 'Agora', price.breakdown.urgency]);
                return rows;
              })().map((row, i) => (
                <div
                  key={i}
          ...
          >
            <span style={{ color: 'var(--orange-600)', marginTop: 2 }}>
              <Icon name="alert" size={18} />
            </span>
            <div style={{ fontSize: 13, color: 'var(--ink-800)', lineHeight: 1.5 }}>
              <strong>Pode variar conforme:</strong> dificuldade real no local, horário (madrugada
              custa mais), tempo de espera.
            </div>
          </div>
        </div>
        <div className="pg-page-foot">
      ...
      <div className="pg-page">
        <div className="pg-page-body">
          <div>
            ...
            <p className="pg-h-sub">
              O destino correto depende do material — alguns têm taxa extra de descarte.
            </p>
          </div>
          <div className="pg-stack pg-stack--sm">
            {opts.map((o) => (
              <button
                key={o.id}
                className={`pg-choice${state.material === o.id ? ' is-active' : ''}`}
          ...
          >
            ...
            <div style={{ fontSize: 13, color: 'var(--ink-800)', lineHeight: 1.5 }}>
              <strong>Materiais perigosos não são aceitos:</strong> tintas, solventes, químicos,
              amianto, lâmpadas, baterias.
            </div>
          </div>
        </div>
        <div className="pg-page-foot">
...
// =====================================================================
const Cacamba2 = ({ go, state, set }: FlowScreenProps) => {
  const sizes = [
    {
      id: 'p3',
      t: '3 m³ · pequena',
      s: '~300 tijolos · pequena reforma',
      price: 'R$ 180',
      popular: false,
    },
    {
      ...
      <div className="pg-page">
        <div className="pg-page-body">
          <div>
            ...
            <p className="pg-h-sub">
              Em dúvida? Use referência: 1 metro cúbico ≈ 1 carrinho de mão.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="pg-stack pg-stack--sm">
              {times.map((o) => (
                <button
                  key={o.id}
                  className={`pg-choice${state.duration === o.id ? ' is-active' : ''}`}
                ...
                >
                  ...
                  <div className="pg-choice-body">
                    <div className="pg-row pg-row--between">
                      ...
                      <span className="pg-mono" style={{ fontSize: 13, fontWeight: 600 }}>
                        {o.extra}
                      </span>
                    </div>
                    <div className="pg-choice-sub">{o.s}</div>
        ...
        <div className="pg-page-foot">
          <button
            className="pg-btn pg-btn--primary pg-btn--block"
            disabled={!state.size || !state.duration}
            onClick={() => go('cacamba-3')}
          >
            Continuar <Icon name="arrow-right" size={18} />
          </button>
        </div>
...
// =====================================================================
const Cacamba3 = ({ go, state, set }: FlowScreenProps) => {
        ...
        <div className="pg-page-body">
          ...
          >
            ...
            <div className="pg-input-wrap">
              ...
              <input
                className="pg-input pg-input--with-icon"
                placeholder="Endereço onde colocar a caçamba"
                value={state.address || ''}
            ...
            <div className="pg-segmented">
              {[
                ...
                <button
                  key={o.id}
                  className={`pg-segmented-item${state.placement === o.id ? ' is-active' : ''}`}
                  onClick={() => set({ placement: o.id })}
                >
                  {o.t}
                </button>
              ))}
            ...
            <div className="pg-helper">
              Verifique se há autorização da prefeitura para colocação em via pública.
            </div>
          </div>
...
// =====================================================================
const Proposals = ({ go, reset }: ScreenProps & { reset: ResetFn }) => {
  const [selected, setSelected] = useStateG<string | null>(null);
  ...
  return (
    <div className="pg-screen" data-screen-label="10 Comparar propostas">
      ...
      <TopBar
        ...
        right={
          <button className="pg-iconbtn" aria-label="Filtrar">
            <Icon name="filter" />
          </button>
        }
      />
      ...
      >
        <div className="pg-row pg-row--between">
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, marginTop: 2 }}>
              Frete · 15 km · Amanhã 09:00
            </div>
          </div>
          <span
            className="pg-tag pg-tag--green"
            style={{ background: 'rgba(34,227,163,0.16)', color: 'var(--green-500)' }}
          >
            <span
              style={{
                width: 6,
        ...
        <div
          className="pg-row"
          style={{ marginTop: 14, gap: 16, fontSize: 12, color: 'rgba(255,255,255,0.65)' }}
        >
          <span>
            <Icon name="ruler" size={13} /> 15 km
          </span>
          <span>
            <Icon name="truck" size={13} /> Caminhão baú
          </span>
          <span>
            <Icon name="users" size={13} /> 1 ajudante
          </span>
        </div>
      </div>
      ...
      <div style={{ padding: '14px 20px 0', flexShrink: 0 }}>
        ...
        <div className="pg-segmented" style={{ marginTop: 8 }}>
          <button
            className={`pg-segmented-item${sort === 'price' ? ' is-active' : ''}`}
            onClick={() => setSort('price')}
          >
            Menor preço
          </button>
          <button
            className={`pg-segmented-item${sort === 'rating' ? ' is-active' : ''}`}
            onClick={() => setSort('rating')}
          >
            Melhor avaliado
          </button>
          <button
            className={`pg-segmented-item${sort === 'time' ? ' is-active' : ''}`}
      ...
      <div className="pg-viewport">
        <div
          style={{ padding: '16px 20px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}
        >
          {/* Range reminder */}
          <div
          ...
          >
            ...
            <div style={{ fontSize: 13, color: 'var(--text-soft)' }}>
              Sua estimativa era <strong style={{ color: 'var(--text)' }}>R$ 280–350</strong>.
              Compare propostas e negocie direto.
            </div>
          </div>
          ...
          {sorted.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="pg-row"
                  style={{ gap: 12, marginTop: p.cheapest || p.fast ? 14 : 0 }}
                >
                  {/* avatar */}
                  <div
                    style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div className="pg-row" style={{ gap: 6 }}>
                      ...
                      {p.verified && (
                        <span title="Verificado" style={{ color: 'var(--green-700)' }}>
                    ...
                    <div
                      className="pg-row"
                      style={{ gap: 8, fontSize: 12, color: 'var(--text-soft)', marginTop: 2 }}
                    >
                      <span>
                        <Icon name="star" size={12} /> {p.rating} ({p.reviews})
                      </span>
                      <span>·</span>
                ...
                <div className="pg-row" style={{ marginTop: 14, gap: 8 }}>
                  ...
                  >
                    {isSelected ? (
                      <>
                        <Icon name="check" size={16} strokeWidth={3} /> Selecionado
                      </>
                    ) : (
                      'Aceitar proposta'
                    )}
...
export { Guincho1, Guincho2, Guincho3, Guincho4, Cacamba1, Cacamba2, Cacamba3, Proposals };
